...
import ReviewModelRequest from "../../models/ReviewRequestModel";
...
export const BookCheckoutPage = () => {
  ...
  const [isLoadingUserReview, setIsLoadingUserReviewLeft] = useState(true);
  ...
  }, [isBookcheckout, getAccessTokenSilently, bookId]);
  ...
  useEffect(() => {
    const fetchUserReviewBook = async () => {
      if (isAuthenticated) {
        ...
        const isUserLeftReviewUrl: string = `http://localhost:8080/api/reviews/secure/user/book?bookId=${bookId}`;
        ...
        const isUserLeftReviewResponse = await fetch(isUserLeftReviewUrl, requestOptions);

        if (!isUserLeftReviewResponse.ok) {
          throw new Error("Something went wrong!");
        }

        const responseJson = await isUserLeftReviewResponse.json();
        setIsReviewLeft(responseJson);
      }

      setIsLoadingUserReviewLeft(false);
    };
  ...
  }, [isAuthenticated, getAccessTokenSilently, isBookcheckout]);
  ...
  const submitReview = async (starInput: number, reviewDescription: string) => {
    ...
    const reviewModelRequest = new ReviewModelRequest(starInput, bookId, reviewDescription);
    const accessToken = await getAccessTokenSilently();
    ...
    const requestOptions = {
      ...
      body: JSON.stringify(reviewModelRequest)
    };
      ...
      <div className="container d-none d-lg-block">
        <div className="row mt-5">
          ...
          <CheckoutAndReviewBox
            ...
            isBookCheckedOut={isBookcheckout}
            checkoutBoook={checkoutBoook}
